...
export async function listContracts(options: ListContractsOptions = {}): Promise<{ data: Contract[]; total: number }> {
  const tableId = await getTableId();
  const result = await listRecords(tableId, options);
  return { data: result.records.map(mapRecord), total: result.total };
}

export async function searchContracts(query: string, filters?: ComplexFilter, extra?: { skip?: number; take?: number }): Promise<{ data: Contract[]; total: number }> {
  const tableId = await getTableId();
  const result = await searchRecords(tableId, {
    query,
    fields: ['ContractCode', 'CustomerName', 'CompanyName', 'Notes'],
    filters,
    skip: extra?.skip,
    take: extra?.take,
  });
